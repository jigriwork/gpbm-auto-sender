  ...
  return (
    <main className="min-h-screen bg-[#f7f7f4] text-black">
      <div className="grid min-h-screen lg:grid-cols-[0.92fr_1.08fr]">
        <section className="flex min-h-[520px] flex-col justify-between bg-black p-6 text-white sm:p-8 lg:min-h-screen lg:p-10">
          <Link href="/" className="flex items-center gap-3">
            ...
            <span>
              ...
              <span className="block text-xs text-neutral-400">Secure SaaS dashboard</span>
            </span>
          </Link>
          <div className="my-12 max-w-xl">
            ...
            <p className="mt-6 text-base leading-7 text-neutral-400">Sign in to manage stores, agents, providers, templates, retries, failed bills, and delivery proof.</p>
            <div className="mt-8 grid gap-3">
              {["Supabase session protected", "Business and role context aware", "Provider secrets stay server-side", "Customer data is redacted where appropriate"].map((item) => (
          ...
          <p className="text-xs text-neutral-500">No public signup. Ask the business admin for an invite.</p>
        </section>
        ...
        <section className="grid place-items-center p-4 sm:p-8">
          <div className="w-full max-w-md">
            <div className="private-card p-6 sm:p-7">
              <div className="mb-6 flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-neutral-600">Use the email and password assigned by your business admin.</p>
                </div>
              ...
              <form onSubmit={submit} className="grid gap-4">
                <label className="grid gap-2 text-sm font-semibold">
                  Email
                  <input value={email} onChange={(event) => setEmail(event.target.value)} className="private-input h-12" placeholder="admin@gpbm.in" type="email" autoComplete="email" required />
                </label>
                <label className="grid gap-2 text-sm font-semibold">
                  Password
                ...
                {message ? <p className="rounded border border-black bg-white p-3 text-sm text-neutral-700">{message}</p> : null}
            ...
            <div className="mt-4 grid gap-3 text-sm text-neutral-600 sm:grid-cols-2">
              ...
              <span className="sm:text-right">Forgot password placeholder</span>
            </div>
            <div className="mt-4 rounded border border-neutral-300 bg-white p-4 text-sm leading-6 text-neutral-600">
              <div className="flex gap-3">
                <Mail size={18} className="mt-0.5 shrink-0 text-black" />
                <p>Need access? Contact the owner/admin for an invite. Public signup is intentionally disabled until tenant onboarding is complete.</p>
              </div>
            </div>
          </div>
        </section>
